...
def resolve_openai_api_key() -> str:
    ...
    if not secret_arn:
        raise RuntimeError("OPENAI_API_KEY or OPENAI_API_KEY_SECRET_ARN must be set")

    sm_client = boto3.client("secretsmanager")
    secret_value = sm_client.get_secret_value(SecretId=secret_arn)
    secret_string = secret_value.get("SecretString", "").strip()
    if not secret_string:
        raise RuntimeError("Secrets Manager entry for OpenAI key is empty")
    ...
    if secret_string.startswith("{"):
        ...
        if not key:
            raise RuntimeError("OPENAI_API_KEY not found in secret JSON payload")
        return key
...
async def health_check():
    return {
        "status": "healthy..everything is running as expected!",
        "use_s3": USE_S3,
