...
resource "aws_iam_policy" "lambda_openai_secret_access" {
  count = var.openai_api_key_secret_arn != "" ? 1 : 0
  name  = "${local.name_prefix}-lambda-openai-secret-access"
  tags  = local.common_tags
...
resource "aws_iam_role_policy_attachment" "lambda_openai_secret_access" {
  count      = var.openai_api_key_secret_arn != "" ? 1 : 0
  role       = aws_iam_role.lambda_role.name
  policy_arn = aws_iam_policy.lambda_openai_secret_access[0].arn
}
...
resource "aws_route53_record" "site_validation" {
  for_each = var.use_custom_domain && var.enable_cloudfront ? {
